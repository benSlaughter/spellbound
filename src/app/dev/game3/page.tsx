...
function scatterPositions(count: number): { top: string; left: string }[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const col = i % cols;
    const baseTop = 10 + (row / rows) * 55;
    const baseLeft = 8 + (col / cols) * 80;
    positions.push({
          ...
          {rocks.map((rock) => {
            ...
            return (
              <motion.button
                ...
                className="absolute w-12 h-12 sm:w-14 sm:h-14 rounded-lg
                  bg-gradient-to-b from-stone-600 to-stone-800
                  text-white font-bold text-xl sm:text-2xl
                  flex items-center justify-center
                  cursor-pointer shadow-lg border border-stone-500/50
